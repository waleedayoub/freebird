...
def run() -> None:
    ...
    rows = db.conn.execute(
        """SELECT s.id, s.image_path, s.species
           FROM sightings s
    ...
    for i, row in enumerate(rows, 1):
        ...
        if vision and vision.is_bird and vision.species:
            analyzed += 1
            # Update sighting species if it doesn't already have one
            if not existing_species:
                ...
                logger.info("  -> Vision: %s (keeping existing: %s)",
                            vision.species, existing_species)
